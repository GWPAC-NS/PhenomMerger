...
#sample_rate = 4096*10
#delta_t = 1.33e-5
def Match(m_1,m_2,num_t,num_hp,num_hc,freq,PN_h_wave, PN_tc, cutoff, merger, style, save,i):
    """
    ...
    num_t = num_t*(m_1+m_2)*(4.92686088e-6)
##### Define interpolation functions

    delta_t = PN_tc[1]-PN_tc[0] #Gives us the step size of the PN
...
#Combine the plus and cross polarizations
    ...
    num_ts = num_ts/(m_1+m_2)/(4.92686088e-6)
####Select match region
#Find the index to start
    def Initial():
        n=0
        for line in num_ts:
            if num_ts[n]<300: #or new_freq[n]<cutoff:
                n+=1
            else:
                return n
    match_i = Initial()
#Index where the modulus is at a max
    if merger == 1:
...
#    print new_freq[0], new_freq[match_i], new_freq[match_f]
    num_ts = num_ts*(m_1+m_2)*(4.92686088e-6)
...
#Graph the cross polarization of the wave
    ...
    #plt.plot(num_tc[match_i:],np.imag(num_phase_shift[match_i:]))
            ...
            plt.xlabel('t (s)')
            plt.ylabel('$hcross $')
            plt.savefig(str(i)+'\HybridMatch.png')
            plt.close()
...
#### Graph the Phase Evolutions
            ...
            plt.savefig(str(i)+'\RMSGraph.png')
            plt.close()

#### Save the information about the waveform into text files
        if save ==2:
            np.savetxt(str(i)+'\Match_RMS_values.txt', [A,r], delimiter = ' ')
